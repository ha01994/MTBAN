...
input_file = "/home/ha01994/mutationTCN/_job%s/%s.a2m"%(jobId, jobId)
...
np.save('/home/ha01994/mutationTCN/_job%s/data_aa'%jobId, translated_seqs) 
...
np.save('/home/ha01994/mutationTCN/_job%s/all_possible_mut_data_aa'%jobId, translated_mutated_seqs)
np.save('/home/ha01994/mutationTCN/_job%s/all_possible_mutations'%jobId, mutations)
...
if os.path.exists('/home/ha01994/mutationTCN/_job%s/weights.npy'%jobId) is False:
    ...
    np.save('/home/ha01994/mutationTCN/_job%s/weights'%jobId, weights)
...
#print(len(mut_seqs))
#print('new_mutations', new_mutations)


new_mutations = np.array(new_mutations)
np.save('/home/ha01994/mutationTCN/_job%s/mutations'%jobId, new_mutations)
...
if flag == True:
    if len(excluded_mutations) == len(mutations):
        with open('/home/ha01994/mutationTCN/_job%s/errorment.txt'%jobId,'w') as fw:
            fw.write('The given mutation(s) correspond to position(s) that is/are not aligned in the multiple sequence alignment: %s'%(','.join(excluded_mutations)))
            exit()
        
    else:
        with open('/home/ha01994/mutationTCN/_job%s/predictions_jobId%s.csv'%(jobId,jobId), 'a') as f:
            f.write('The following mutations correspond to positions that are not aligned in the multiple sequence alignment:\n') 
